...
read_expression_data <- function(file.expr,
  ...
  # method_specific_expr    method used for quantifying annotation specific expression. OPTIONS:
  #                         "tstat": t-statistics for annotation type specific expression.
  #                        "average": average gene expression values (unscaled). Data will be *SCALED* in this function (zscores).
  ...
  if (!is.null(genes.filter)) {
    bool.expr_genes_filtered <- df.expr %>% pull(UQ(rlang::sym(col_genes))) %in% genes.filter
    print(sprintf("Number of genes filtered out: %s (genes not contained in genes.filter)", sum(bool.expr_genes_filtered)))
    df.expr <- df.expr %>% filter(bool.expr_genes_filtered)
  }
  ...
  if (method_specific_expr=="tstat") {
    print("Got 'tstat' specific expression data.")
  } else if (method_specific_expr=="average") {
    print("Got 'average' specific expression data. Will scale data across genes (mean=0 and sd=1 for each gene)..")
    ...
    stop(sprintf("Got wrong 'method_specific_expr' argument: %s", method_specific_expr))
  }
  ...
  if (pos_transformation=="square") {
    df.expr.rp <- df.expr^2
  } else if (pos_transformation=="abs") {
    df.expr.rp <- abs(df.expr)
    ...
    stop(sprintf("Got wrong 'pos_transformation' argument: %s", pos_transformation))
  }
