...
```{r}
dir.ldfiles <- "/Users/djw472/data/rolypoly/EUR_LD_FILTERED_NONAN_R" # Linkage disequilibrium files
```
...
```{r}
file.expr <- "../data/expression/arc_lira-celltype_expr_ttest.hsapiens_orthologs.csv"
...
df.gwas.rp.test <- df.gwas.rp %>% slice(1:10000)
...
```{r}
library(doParallel)
registerDoParallel() # half of the number of cores 
getDoParWorkers()
```
...
# message = FALSE
rp <- rolypoly_roll(
  gwas_data = as.data.frame(df.gwas.rp.test),
