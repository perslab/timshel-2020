...
list.run_parameters <- list(POS_TRANSFORMATION=POS_TRANSFORMATION,
...
if (FLAG_TEST_RUN) {
  ...
  df.gwas.rp <- df.gwas.rp %>% slice(1:100)
}
...
files <- list.files(file.path(dir.sc_genetics.data, "expression"), recursive=T, pattern="*hsapiens_orthologs*")
...
runtime.first_run <- system.time(rp.first_run <- rolypoly_roll(
  ...
  bootstrap_iters = 50, # use at least 200
  gwas_link_parallel = F,
...
wrapper.run_rolypoly <- function(idx.list, list) {
  ### INPUT
  ...
  print(sprintf("========== #%s/#%s | wrapper.run_rolypoly | list element = %s ==========", idx.list, n.elements, name.list)) # prints "character(0)" if list has no names (i.e. names(list) is NULL)
  
  df.expr <- list[[idx.list]] # get expression data frame
  
  rp <- NULL
  runtime <- NULL # default return value
  
  runtime <- system.time(
    try({ # if something fails here, we don't want to stop everything.
      rp <- rolypoly_roll(
        rolypoly=rp.first_run, # RolyPoly object
        block_data = as.data.frame(df.expr),
        bootstrap_iters = 500, # use at least 200
        bootstrap_parallel = T)
      })
  ) # system.time(): returns a names numerical vector of class "proc_time". # names: "user.self"  "sys.self"   "elapsed"    "user.child" "sys.child" 
  list.res <- c("rp"=list(rp), "runtime"=list(runtime)) # elements must be enclosed in list() to keep their native data structure/'integrity'
  return(list.res)
}

print("Running wrapper.run_rolypoly with the following files:")
print(files.paths)

list.rp_wrapper <- lapply(seq_along(list.df_expr), wrapper.run_rolypoly, list=list.df_expr) # "list" is here the extra (named) argument to wrapper.run_rolypoly()
