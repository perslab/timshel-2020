...
system(paste(ldsc_path, "ldsc.py -h", sep=""))
system(paste(ldsc_path, "munge_sumstats.py -h", sep=""))

### CREATING LD SCORES ###

setwd(paste(ldsc_path, "files/eur_ref_ld_chr", sep=""))
system("gzcat 22.l2.ldscore.gz | head | column -t") 
...
command <- paste(ldsc_path, "ldsc.py",
                 " --bfile ", ldsc_path, "files/1kg_eur/22",
                 " --l2",
                 " --ld-wind-cm 1",
                 " --out 22",
                 sep="")
...
old_ldsc <- read.table(paste(ldsc_path, "files/eur_w_ld_chr/22.l2.ldscore.gz", sep=""),
                       header=TRUE, colClasses=c("integer", "character", "integer", "numeric",
                       "numeric", "numeric"))                       
head(old_ldsc)

both_ldsc <- merge(old_ldsc, new_ldsc, by="SNP")
cor.test(both_ldsc$L2.x, both_ldsc$L2.y) 

system("rm -f 22.*")

### ESTIMATING INTERCEPTS AND CORRECTING STANDARD ERRORS ###

system("gzcat EA2_excl_23andMe_noGC.meta.gz | head")

command <- paste(ldsc_path, "munge_sumstats.py",
                 " --sumstats EA2_excl_23andMe_noGC.meta.gz",
                 " --out EA2_excl_23andMe_noGC",
                 " --merge-alleles ", ldsc_path, "files/w_hm3.snplist",
...
command <- paste(ldsc_path, "ldsc.py",
                 ...
                 " --ref-ld-chr ", ldsc_path, "files/eur_ref_ld_chr/",
                 " --w-ld-chr ", ldsc_path, "files/eur_ref_ld_chr/",
                 " --out EA2_excl_23andMe_noGC",
                 sep="")
...
command <- paste(ldsc_path, "munge_sumstats.py",
                 " --sumstats EGG_HC_DISCOVERY.txt.gz",
                 " --out HC",
                 " --merge-alleles ", ldsc_path, "files/w_hm3.snplist",
...
command <- paste(ldsc_path, "munge_sumstats.py",
                 " --sumstats pgc.cross.BIP11.2013-05.txt.gz",
                 " --out BIP",
                 " --merge-alleles ", ldsc_path, "files/w_hm3.snplist",
                 " --N-cas 6990",
                 " --N-con 4820",
...
command <- paste(ldsc_path, "ldsc.py",
                 " --h2 EA2_excl_23andMe_noGC.sumstats.gz",
                 " --ref-ld-chr ", ldsc_path, "files/1000G_EUR_Phase3_baseline/baseline.",
                 " --w-ld-chr ", ldsc_path, "files/weights_hm3_no_hla/weights.",
                 " --overlap-annot",
                 " --frqfile-chr ", ldsc_path, "files/1000G_Phase3_frq/1000G.EUR.QC.",
                 " --print-coefficients",
...
tissues <- read.table(paste(ldsc_path, "files/1000G_Phase3_cell_type_groups/names", sep=""),
                    header=TRUE)
...
for(i in 1:10){
	...
	command <- paste(ldsc_path, "ldsc.py",
	                 " --h2 EA2_excl_23andMe_noGC.sumstats.gz",
	                 " --ref-ld-chr ", ldsc_path, "files/1000G_Phase3_cell_type_groups/",
	                 "cell_type_group.", i, ".,",
	                 ldsc_path, "files/1000G_EUR_Phase3_baseline/baseline.",
	                 " --w-ld-chr ", ldsc_path, "files/weights_hm3_no_hla/weights.",
	                 " --overlap-annot",
	                 " --frqfile-chr ", ldsc_path, "files/1000G_Phase3_frq/1000G.EUR.QC.",
	                 " --print-coefficients",
...
system("head EA2_excl_23andMe_noGC_Adrenal_Pancreas.results")

command <- paste("head -n1 EA2_excl_23andMe_noGC_Adrenal_Pancreas.results >>",
...
for(i in 1:22){
	print(i)
	CNS <- read.table(paste(ldsc_path, "files/1000G_Phase3_cell_type_groups/",
...
for(i in 1:22){
	...
	command <- paste(ldsc_path, "ldsc.py",
	                 " --l2",
	                 " --bfile ", ldsc_path, "files/1000G_EUR_Phase3_plink/1000G.EUR.QC.", i,
	                 " --ld-wind-cm 1",	            
	                 " --annot DNase_fBrain.", i, ".annot.gz",
	                 " --out DNase_fBrain.", i,
	                 " --print-snps ", ldsc_path, "files/1000G_EUR_Phase3_baseline/print_snps.txt",
	                 sep="")
	system(command)
