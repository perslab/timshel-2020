...
# ======================= DEPICT ======================= #
df.category.depict <- read_tsv("name_original	category
...
### Cleaning names
tmp_clean1 <- stringr::str_replace_all(df.category.depict$name_original, pattern="A(.*)(\\d+)\\.", replacement="") # e.g. "Precursor.Cells..B.Lymphoid", "B.Lymphocytes"...
# tmp_clean1
...
# sort(tmp_clean3)
### Set new names
df.category.depict <- df.category.depict %>% 
  mutate(
...
# ======================= GTEX ======================= #
df.category.gtex <- read_tsv("SMTS	SMTSD	sub_tissue	tissue	category
...
### Set new names
df.category.gtex <- df.category.gtex %>% 
  mutate(
...
# df.category.gtex %>% distinct(category) # --> 9
# df.category.gtex %>% distinct(sub_category) # --> 30
