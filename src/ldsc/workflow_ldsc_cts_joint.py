...
dict_cts_conditional = {"celltypes.mousebrain.all":
						{"file_cts":"/projects/timshel/sc-genetics/sc-genetics/src/ldsc/cts_files/celltypes.mousebrain.all.ldcts.txt",
						"dataset":"mousebrain",
						"conditional_annotations":list_cond_annotations_mb},
 					 "celltypes.tabula_muris.all":
 					  	{"file_cts":"/projects/timshel/sc-genetics/sc-genetics/src/ldsc/cts_files/celltypes.tabula_muris.all.ldcts.txt",
...
for prefix_genomic_annot, param_dict in dict_cts_conditional.items():
	...
	# for cond_annotation in param_dict["conditional_annotations"]:
		list_joint = [get_cond_ref_ld_chr_name(cond_annotation, param_dict["dataset"]) for cond_annotation in param_dict["conditional_annotations"]] # get list of strings
		cond_ref_ld_chr_name_joint = ",".join(list_joint) # join by comma to parse this directly to LDSC
		joint_annotation_id_string = "--".join(param_dict["conditional_annotations"]) # set name for output file. Use "--" as separator between annotation IDs. ALT "_-_" 
		fileout_prefix = "/projects/timshel/sc-genetics/sc-genetics/out/out.ldsc/{prefix_genomic_annot}__{gwas}__JOINT__{joint_annotation_id_string}".format(gwas=gwas, prefix_genomic_annot=prefix_genomic_annot, joint_annotation_id_string=joint_annotation_id_string)
        ...
        --ref-ld-chr /projects/timshel/sc-genetics/ldsc/data/baseline_v1.1/baseline.,{ldsc_all_genes_ref_ld_chr_name},{cond_ref_ld_chr_name_joint} \
        ...
        --out {fileout_prefix}""".format(
			...
			cond_ref_ld_chr_name_joint=cond_ref_ld_chr_name_joint,
