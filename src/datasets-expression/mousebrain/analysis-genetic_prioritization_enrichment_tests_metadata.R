...
# 
# file.ldsc_cts <- sprintf("/projects/timshel/sc-genetics/sc-genetics/out/out.ldsc/%s__SCZ_Pardinas2018.cell_type_results.txt", genomic_annotation_prefix)
# df.ldsc_cts <- load_ldsc_cts_results(file.ldsc_cts, dataset_prefix)
# df.ldsc_cts <- df.ldsc_cts %>% filter(sem=="sem_mean")
# 
# file.ldsc_cts <- sprintf("/projects/timshel/sc-genetics/sc-genetics/out/out.ldsc/%s__ASD_iPSYCH_PGC_Grove2018.cell_type_results.txt", genomic_annotation_prefix)
# df.ldsc_cts <- load_ldsc_cts_results(file.ldsc_cts, dataset_prefix)
# df.ldsc_cts <- df.ldsc_cts %>% filter(sem=="sem_mean")
...
### Separate rows
df.ldsc_cts.region <- df.ldsc_cts %>% 
...
### Count
df.ldsc_cts.region.count <- df.ldsc_cts.region %>% count(Region) %>% arrange(n)
df.ldsc_cts.region.count
...
# filter.gwas <- "HEIGHT_UKBB_Loh2018"

cols2test <- c("TaxonomyRank4", "Neurotransmitter_class", "Region", "Class")
list.res.gwas <- list()
for (name_gwas in filter.gwas) {
  ...
  for (colname_test_for_enrichment in cols2test) {
    ...
    if (colname_test_for_enrichment == "Region") {
      df.dataset <- df.ldsc_cts.region 
    } else {
    ...
    # df.wtest_res <- run_enrichment(df.dataset, colname_test_for_enrichment, colname_statistic, do_empirical_pvalue=T, run_parallel=T)
...
df.wtest_multi_gwas
# df.wtest_multi_gwas.all <- df.wtest_multi_gwas
